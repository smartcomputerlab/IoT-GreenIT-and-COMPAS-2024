...
{ 
  ...
  if(sent_valid(stemp,ntemp,0.1))
    ...
    if(lora_idle == true)  // is true when TX is done !
      ...
      sdp.pay.flag[0]=0xF0; sdp.pay.flag[1]=0x00;
      Serial.printf("Channel flags: %2.2X, %2.2X\n", sdp.pay.flag[0],sdp.pay.flag[1]);
      ...
      Radio.Send( (uint8_t *)sdp.frame,58 ); //send the package out 
